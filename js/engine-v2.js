...
window.tickV2 = function() {
    try {
        ...
        const mCtx = window._v2MCvs.getContext('2d'); 
        const tCtx = window._v2TrailCvs.getContext('2d'); 
        const fCtx = window._v2FinalCvs.getContext('2d');
        const bounds = 250; const blockSize = window.IS_MOBILE ? 14 : 20;
        window._v2Ticks = (window._v2Ticks || 0) + 1;

        // 2. Physics & Biological Cone Conversion
        window.v2LiveBalls.forEach(b => {
            ...
            b.wavelength = 550; 
            if (window.windingToWavelength) { let w = window.windingToWavelength(b.winding); if(w && !isNaN(w.nm)) b.wavelength = w.nm; }
        ...
        // 3. The Waterfall Sequence (Shift Down)
        if (window._v2Ticks % 4 === 0 && CE && CE.interfereCones) {
            ...
            for(let x=0; x<W; x+=blockSize) {
                ...
                mCtx.fillStyle = hex; mCtx.fillRect(x, 0, blockSize, blockSize); 
                mCtx.strokeStyle = 'rgba(0,0,0,0.5)'; mCtx.strokeRect(x, 0, blockSize, blockSize);
            }
        }

        // 4. Assembly & Trail Rendering
        ...
        window.v2LiveBalls.forEach(b => {
            ...
            if(b.path.length>1){ 
                tCtx.beginPath(); tCtx.moveTo(b.path[b.path.length-2].x+W/2, b.path[b.path.length-2].y+H/2); 
                tCtx.lineTo(b.x+W/2, b.y+H/2); tCtx.strokeStyle=h; tCtx.stroke(); 
            }
        });
        
        fCtx.globalCompositeOperation = 'screen'; fCtx.drawImage(window._v2TrailCvs, 0, 0);
        fCtx.globalCompositeOperation = 'source-over';
        window.v2LiveBalls.forEach(b => { fCtx.beginPath(); fCtx.arc(b.x+W/2, b.y+H/2, 4, 0, Math.PI*2); fCtx.fillStyle='#ffffff'; fCtx.fill(); });

        // 5. Push to DOM if active
...
window.initV2 = function() {
    function loop() { 
        const root = document.getElementById('v2-root');
        if (root && window.getComputedStyle(root).display !== 'none') window.tickV2(); 
        window._v2Loop = requestAnimationFrame(loop); 
    }
