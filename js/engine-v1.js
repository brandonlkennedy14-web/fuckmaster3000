...
window.tickV1 = function() {
    try {
        ...
        const bCtx = window._v1BgCvs.getContext('2d'); const tCtx = window._v1TrailCvs.getContext('2d'); const fCtx = window._v1FinalCvs.getContext('2d');
        ...
        if (window._v1Ticks % 3 === 0) {
            ...
            for(let i=0;i<7;i++){ bCtx.fillStyle = sorted[i%sorted.length].hex; bCtx.fillRect(0, Math.floor(stripH*i), blockW, Math.ceil(stripH)); }
        }

        fCtx.globalCompositeOperation = 'source-over'; fCtx.drawImage(window._v1BgCvs, 0, 0);
        ...
        window.v1LiveBalls.forEach(b => {
            if(b.path.length>1){ tCtx.beginPath(); tCtx.moveTo(b.path[b.path.length-2].x+W/2, b.path[b.path.length-2].y+H/2); tCtx.lineTo(b.x+W/2, b.y+H/2); tCtx.strokeStyle=b.hex; tCtx.stroke(); }
        });
...
window.initV1 = function() {
    function loop() { const root = document.getElementById('v1-root'); if (root && window.getComputedStyle(root).display !== 'none') window.tickV1(); window._v1Loop = requestAnimationFrame(loop); }
